...
local ApolloFolder = "Apollo"
local ConfigurationFolder = ApolloFolder.."/Configurations"
...
local ApolloLibrary = {
	...
	Theme = {
		Default = {
			TextFont = "Default", -- Default will use the various font faces used across Apollo
		...
		Light = {
			TextFont = "Gotham", -- Default will use the various font faces used across Apollo
...
local Apollo = game:GetObjects("rbxassetid://10804731440")[1]

Apollo.Enabled = false


if gethui then
	Apollo.Parent = gethui()
elseif syn.protect_gui then 
	syn.protect_gui(Apollo)
	Apollo.Parent = CoreGui
elseif CoreGui:FindFirstChild("RobloxGui") then
	Apollo.Parent = CoreGui:FindFirstChild("RobloxGui")
else
	Apollo.Parent = CoreGui
end
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == Apollo.Name and Interface ~= Apollo then
			Interface.Enabled = false
			Interface.Name = "Apollo-Old"
		end
	end
else
...
local Main = Apollo.Main
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList

Apollo.DisplayOrder = 100
...
local Notifications = Apollo.Notifications

local SelectedTheme = ApolloLibrary.Theme.Default

function ChangeTheme(ThemeName)
	SelectedTheme = ApolloLibrary.Theme[ThemeName]
	for _, obj in ipairs(Apollo:GetDescendants()) do
	...
	Apollo.Main.BackgroundColor3 = SelectedTheme.Background
	Apollo.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	Apollo.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	Apollo.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow

	Apollo.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	Apollo.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	Apollo.Main.Topbar.Theme.ImageColor3 = SelectedTheme.TextColor
...
local function LoadConfiguration(Configuration)
	...
	for FlagName, FlagValue in next, Data do
		if ApolloLibrary.Flags[FlagName] then
			spawn(function() 
				if ApolloLibrary.Flags[FlagName].Type == "ColorPicker" then
					ApolloLibrary.Flags[FlagName]:Set(UnpackColor(FlagValue))
				else
					if ApolloLibrary.Flags[FlagName].CurrentValue or ApolloLibrary.Flags[FlagName].CurrentKeybind or ApolloLibrary.Flags[FlagName].CurrentOption or ApolloLibrary.Flags[FlagName].Color ~= FlagValue then ApolloLibrary.Flags[FlagName]:Set(FlagValue) end
				end    
			end)
		else
			ApolloLibrary:Notify({Title = "Flag Error", Content = "Apollo was unable to find '"..FlagName.. "'' in the current script"})
		end
	end
end
...
local function SaveConfiguration()
	...
	local Data = {}
	for i,v in pairs(ApolloLibrary.Flags) do
...
function ApolloLibrary:Notify(NotificationSettings)
	spawn(function()
		...
		if NotificationSettings.Actions then
			for _, Action in pairs(NotificationSettings.Actions) do
				...
				if SelectedTheme ~= ApolloLibrary.Theme.Default then
				...
				NewAction.MouseButton1Click:Connect(function()
					...
					if not Success then
						print("Apollo | Action: "..Action.Name.." Callback Error " ..tostring(Response))
					end
...
function Hide()
	Debounce = true
	ApolloLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping K", Duration = 7})
...
function ApolloLibrary:CreateWindow(Settings)
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "Apollo Interface Suite"
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "by Sirius"
	if Settings.LoadingTitle ~= "Apollo Interface Suite" then
		LoadingFrame.Version.Text = "Apollo UI"
	end
	...
	pcall(function()
		...
		if not isfolder(ApolloFolder.."/".."Configuration Folders") then
	...
	if Settings.Discord then
		if not isfolder(ApolloFolder.."/Discord Invites") then
			makefolder(ApolloFolder.."/Discord Invites")
		end
		if not isfile(ApolloFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if request then
				request({
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				writefile(ApolloFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"Apollo RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		else
	...
	if Settings.KeySystem then
		...
		if not isfolder(ApolloFolder.."/Key System") then
			makefolder(ApolloFolder.."/Key System")
		end
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("Apollo | "..Key.." Error " ..tostring(Response))
				end
			end
		end
		...
		if isfile(ApolloFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			for _, MKey in ipairs(Settings.KeySettings.Key) do
				if string.find(readfile(ApolloFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension), MKey) then
					Passthrough = true
				end
			end
		end
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				if KeyFound then 
					...
					if Settings.KeySettings.SaveKey then
						if writefile then
							writefile(ApolloFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, FoundKey)
						end
						ApolloLibrary:Notify({Title = "Key System", Content = "The key for this script has been saved successfully"})
					end
				else
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("Apollo | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
					ApolloLibrary.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
				end
			end
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					print("Apollo | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					wait(0.5)
					Input.Title.Text = InputSettings.Name
		...
		function Tab:CreateDropdown(DropdownSettings)
			...
			for _, Option in ipairs(DropdownSettings.Options) do
				...
				DropdownOption.Interact.MouseButton1Click:Connect(function()
					...
					if not Success then
						...
						print("Apollo | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						Dropdown.Title.Text = DropdownSettings.Name
						TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
					ApolloLibrary.Flags[DropdownSettings.Flag] = DropdownSettings
				end
			end
		...
		function Tab:CreateKeybind(KeybindSettings)
				...
				elseif KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then -- Test
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("Apollo | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
							wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					ApolloLibrary.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("Apollo | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
					wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
					TweenService:Create(Toggle.UIStroke, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {Transparency = 0}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
					ApolloLibrary.Flags[ToggleSettings.Flag] = ToggleSettings
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
					...
					if Dragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								print("Apollo | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
								wait(0.5)
								Slider.Title.Text = SliderSettings.Name
								TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					ApolloLibrary.Flags[SliderSettings.Flag] = SliderSettings
				end
			end
...
function ApolloLibrary:Destroy()
	Apollo:Destroy()
end
...
function ApolloLibrary:LoadConfiguration()
	if CEnabled then
		pcall(function()
			if isfile(ConfigurationFolder .. "/" .. CFileName .. ConfigurationExtension) then
				LoadConfiguration(readfile(ConfigurationFolder .. "/" .. CFileName .. ConfigurationExtension))
				ApolloLibrary:Notify({Title = "Configuration Loaded", Content = "The configuration file for this script has been loaded from a previous session"})
			end
		end)
	end
end

task.delay(3.5, ApolloLibrary.LoadConfiguration, ApolloLibrary)
if Apollo:FindFirstChild("Notice") then
	Apollo.Notice.Visible = true
	Apollo.Notice.Interact.MouseButton1Click:Connect(function()
		Apollo.Notice.Visible = false
	end)
end

	return ApolloLibrary
